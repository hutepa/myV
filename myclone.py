...
def clone(deploy_settings, ip_settings):
    ...
    try:
        si = SmartConnectNoSSL(host=deploy_settings["vserver"], user=deploy_settings["username"],
    ...
    destfolder = get_obj(content, [vim.Folder], 'Cloned')

    cluster = get_obj(content, [vim.ClusterComputeResource], 'HyperFlex')
